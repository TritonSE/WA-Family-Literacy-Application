...
func (db *UserDatabase) CreateUser(ctx context.Context, user models.User) (string, error) {
	...
	err := db.Conn.QueryRowEx(ctx, "INSERT INTO users (id, name, email, in_san_diego) VALUES ($1, $2, $3, $4) RETURNING id", nil, user.ID, user.Name, user.Email, user.InSanDiego).Scan(&id)
	if err != nil {
